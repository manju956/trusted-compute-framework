...
def get_enclave_public_info(sealed_data):
    """
    Return information about the enclave
    """
    signup_obj = enclave.SignupInfoSingeton()
    return signup_obj.UnsealEnclaveData(sealed_data)
...
# -----------------------------------------------------------------
def verify_enclave_info(enclave_info, mr_enclave, signup_cpp_obj):
    """
...
def create_signup_info(nonce):
    ...
    if not enclave.is_sgx_simulator():
        ...
        status = verify_enclave_info(json.dumps(signup_info), mr_enclave, signup_cpp_obj)
        if status != 0:
